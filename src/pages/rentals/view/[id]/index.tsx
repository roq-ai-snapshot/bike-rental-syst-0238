            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Rental Start"
                  text={
                    data?.rental_start ? format(parseISO(data?.rental_start as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />

                <FormListItem
                  label="Rental End"
                  text={data?.rental_end ? format(parseISO(data?.rental_end as unknown as string), 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('bike', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Bike"
                    text={
                      <Link as={NextLink} href={`/bikes/view/${data?.bike?.id}`}>
                        {data?.bike?.status}
                      </Link>
                    }
                  />
