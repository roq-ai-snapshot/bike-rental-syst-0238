  ...
  const columns: ColumnType[] = [
    {
      id: 'rental_start',
      header: 'Rental Start',
      accessorKey: 'rental_start',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_start ? format(parseISO(record?.rental_start as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'rental_end',
      header: 'Rental End',
      accessorKey: 'rental_end',
      cell: ({ row: { original: record } }: any) =>
        record?.rental_end ? format(parseISO(record?.rental_end as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('bike', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'bike',
          header: 'Bike',
          accessorKey: 'bike',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/bikes/view/${record.bike?.id}`}>
              {record.bike?.status}
            </Link>
          ),
        }
